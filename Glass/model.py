...
class DeepGlassNet(nn.Module):
    def __init__(self, num_components, embedding_dim, fm_dim,num_heads, out_dim,dropout_rate=0.1, use_time_attn=False):
        ...
        # 3. Self attention
        if use_time_attn:
            self.attention_layer = TimeParamMultiheadAttention(embed_dim=embedding_dim, num_heads=self.num_heads, use_learned_pos_emb=True)
        else:
